...
if (isset($_GET['feed']) && $_GET['feed'] == "yes") {
	...
	if(!zen_yahoo_fwrite()) {
		...
		if (YAHOO_ASA == 'true') {
		$products_query = "SELECT distinct(p.products_id), p.products_model, pd.products_name, pd.products_description, p.products_image, p.products_tax_class_id, p.products_price_sorter, p.products_upc, p.products_isbn, s.specials_new_products_price, s.expires_date, GREATEST(p.products_date_added, IFNULL(p.products_last_modified, 0), IFNULL(p.products_date_available, 0)) AS base_date, m.manufacturers_name, p.products_quantity, pt.type_handler, p.products_weight, p.products_condition
		...
		$products_query = "SELECT distinct(p.products_id), p.products_model, pd.products_name, pd.products_description, p.products_image, p.products_tax_class_id, p.products_price_sorter, s.specials_new_products_price, s.expires_date, GREATEST(p.products_date_added, IFNULL(p.products_last_modified, 0), IFNULL(p.products_date_available, 0)) AS base_date, m.manufacturers_name, p.products_quantity, pt.type_handler, p.products_weight
										 FROM " . TABLE_PRODUCTS . " p
											 LEFT JOIN " . TABLE_MANUFACTURERS . " m ON (p.manufacturers_id = m.manufacturers_id)
											 LEFT JOIN " . TABLE_PRODUCTS_DESCRIPTION . " pd ON (p.products_id = pd.products_id)
											 LEFT JOIN " . TABLE_PRODUCT_TYPES . " pt ON (p.products_type=pt.type_id)
			...
			while (!$products->EOF && !$max_limit) { // run until end of file or until maximum number of products reached
				...
				if (numinix_categories_check((YAHOO_POS_CATEGORIES), $products->fields['products_id'], 1) == true && numinix_categories_check((YAHOO_NEG_CATEGORIES), $products->fields['products_id'], 2) == false) { // check to see if category limits are set.  If so, only process for those categories.
					...
					if ($price > 0) {
						...
						/**for price grabber**/
						$output['Unique-Retailer-SKU'] = $products->fields['products_id'];
						$output['Manufacturer-Name'] = zen_yahoo_cleaner($products->fields['manufacturers_name']);
						$output['Product-Title'] = zen_yahoo_cleaner($products->fields['products_name']);
						$output['Categorization'] = $bread_crumbs;
						...
						$output['Detailed-Description'] = zen_yahoo_cleaner($products->fields['products_description']);
						$output['Selling-Price'] = is_numeric($products->fields['products_price_sorter']) ? $products->fields['products_price_sorter'] : 'NULL';
						$output['Condition'] = 'New';
						$output['Availability'] = 'Yes';
						
						/* Sometimes Required for price grabber*/
						
						$output['Weight'] = is_numeric($products->fields['products_weight']) ? $products->fields['products_weight'] : 'NULL';
