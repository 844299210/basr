...
//$cate_arr = array(17,16,1068,1071,1003,1070,1098,24,1067,1073);
$cate_id = $_GET['cate_id'];
if(empty($cate_id)){
	echo "cate_id不能为空";exit;
}
$cate_arr = array($cate_id);
$str = implode(',',$cate_arr);
foreach($cate_arr as $st=>$qq){
	$arr = array_unique(get_categories_son_ids($qq,1));
	if($arr){
		if($str){
			$str .= ','.implode(',',$arr);
		}else{
...
$exel_columns = array ('A', 'B', 'C', 'D', 'E', 'F', 'G', 'H', 'I', 'J', 'K', 'L','M','N');
$pricegrabber_title = array ('id', 'title', 'description', 'condition', 'price', 'availability', 'link', 'image link', 'mpn', 'brand','tax','shipping','shipping weight','product type');
...
$query = "SELECT distinct(p.products_id), p.products_model, pd.products_name, pd.products_description, p.products_image, p.products_tax_class_id, p.products_price_sorter,ptc.categories_id, s.specials_new_products_price, s.expires_date, GREATEST(p.products_date_added, IFNULL(p.products_last_modified, 0), IFNULL(p.products_date_available, 0)) AS base_date, m.manufacturers_name, p.products_quantity, pt.type_handler, p.products_weight, p.products_weight_for_view,p.products_SKU
										 ...
										 WHERE p.products_status = 1
											 ...
											 AND p.status = 0
											 AND p.products_id in (select products_id from products_to_categories where categories_id in ($str))
