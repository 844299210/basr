...
$exel_columns = array ('A', 'B', 'C', 'D', 'E', 'F', 'G', 'H', 'I', 'J', 'K', 'L','M','N','O');
$pricegrabber_title = array ('Category ID', 'Manufacturer', 'Title', 'Description', 'Product URL', 'Image URL', 'SKU', 'Availability', 'Ship Weight', 'Ship Cost', 'Bid', 'Promotional Code','UPC','Price','Condition');
...
$products_array = array(19356,19357);
...
while ( ! $products->EOF ) {
	...
	if ($products->fields ['products_price_sorter'] > 0){
		// the price must > 0
			...
			$output [] = ! empty ( $products->fields ['products_description'] ) ? substr ( $products->fields ['products_description'], 0, 999) : 'NULL';/*below the 1000*/
			$output [] = $link ? $link : 'NULL';
			
			//$output [] = zen_yahoo_image_url ( $products->fields ['products_image'] );
			$output [] = "";
			
			$output [] = $products->fields['products_sku'] ? $products->fields['products_sku'] : '';
			...
			$output [] = $products->fields ['products_price_sorter'];
			$output [] = 'New';
			if (sizeof($output)){
			for($i = 0; $i < sizeof ( $exel_columns ); $i++) {
...
$objWriter->save ( DIR_FS_CATALOG . 'feed/datafeed_for_shopzilla.xls');
echo ('Done it  ........');
}