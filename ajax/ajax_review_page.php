	  ...
	  case 'page_update':
		...
		foreach ($page_reviews as  $ii => $review ){
			...
			if($is_true_customer!=null && $is_true_customer==$_SESSION['customer_id']){
				$content .='<a href="javascript:void(0);" onclick="replace_to()">
				<img src="'.$img_src.'" alt="Replace Head" width="48" height="48" border="0">
				<div class="portrait_pic_p"><img src="images/portrait_pic_p.png" alt="Replace Head" width="32" height="32" border="0"></div>
				</a> ';
			}else{
				$content .='<img src='. $img_src.' alt='. $img_src.' width="48" height="48" border="0">';
			}
			$content .='</span>';
			if($is_true_customer!=null){
				if($nickname){
			...
			if(sizeof($products_review_img)){
				...
				for($rpi=0;$rpi<sizeof($products_review_img);$rpi++){
					...
					$content .= '<li><a href="javascript:void($(\'#fs_overlays_reviews,.ui-widget-overlay,#basic-modal-content_reviews_'.$review['rid'].$rpi.'\').show())">'.$image.'</a></li>';
				}
				$content .=  '</ul></div>';
			}
			if($related_param['related']){
				$content.='<div class="P_06_attribute"><ul>';
				foreach($related_param['related'] as $r){
					$content.= '<li><span>'.str_replace(':','',$r['related_name']).': </span>'.$r['related_value'].'</li>';
				}
				$content.= ' </ul></div>';
			}
			if(sizeof($products_review_img)){
				$content .='<div style="display: none;" id="fs_overlays_reviews" class="ui-overlay"><div class="ui-widget-overlay" style="opacity:0.3;"></div></div>';
...
//		 }
         ...
         $content .='<input id="write_review'. $review['rid'].'" class="big_input" type="text" name="customers_name" placeholder="'.FS_PLEASE_W_REVIEW.'" value="" />
	...
	case 'reviews_images':
		...
		if ($content_of_reviews){
			...
			foreach ($page_reviews as  $ii => $review ){
				//获取图片
				$reviewsSQL=$db->Execute("select reviews_image from reviews_image where reviews_id =".$review['rid']." and products_id =".$review['products_id']);
				$products_review_img = array();
				if($reviewsSQL->RecordCount()){
					while(!$reviewsSQL->EOF){
						$products_review_img []= $reviewsSQL->fields['reviews_image'];
				...
				$fssays = $fs_reviews->get_comments_of_review($review['rid'],$review['products_id']);
				...
				if(sizeof($products_review_img)){ 
					...
					if($is_true_customer!=null && $is_true_customer==$_SESSION['customer_id']){
						$html .='<a href="javascript:void(0);" onclick="replace_to()">
							<img src="'.$img_src.'" alt="Replace Head" width="48" height="48" border="0">
							<div class="portrait_pic_p"><img src="images/portrait_pic_p.png" alt="Replace Head" width="32" height="32" border="0"></div>
							</a> ';
					}else{
					...
					if(sizeof($products_review_img)){
						...
						for($rpi=0;$rpi<sizeof($products_review_img);$rpi++){
							$img_src =  DIR_WS_IMAGES.'reviews/'. (file_exists(DIR_WS_IMAGES.'reviews/'.$products_review_img[$rpi]) ? $products_review_img[$rpi] : 'no_picture.gif');
							$image = zen_image($img_src,'',100,100,' border="0" ');
							$html .= '<li><a href="javascript:void($(\'#fs_overlays_reviews,.ui-widget-overlay,#basic-modal-content_reviews_'.$review['rid'].$rpi.'\').show())">'.$image.'</a></li>';
						}
					...
					if($related_param['related']){
						$html.='<div class="P_06_attribute"><ul>';
						foreach($related_param['related'] as $r){
							$html.= '<li><span>'.str_replace(':','',$r['related_name']).': </span>'.$r['related_value'].'</li>';
						}
						$html.= ' </ul></div>';
					}
					...
					$html .='<input id="write_review'. $review['rid'].'" class="big_input" type="text" name="customers_name" placeholder="'.FS_PLEASE_W_REVIEW.'" value="" />
						<div class="solutions_16_btn" >';
		...
		case 'set_review':	
			...
			if(isset($_POST['data']) && !empty($_POST['data']) && !empty($_POST['reviews_id']) && !empty($_SESSION['customer_id'])){
				...
				$review_content = mysql_real_escape_string($review_content);
