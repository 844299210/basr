...
class shoppingCart extends base {
  ...
  function restore_contents() {
    ...
    if (is_array($this->contents)) {
      ...
      //验证get条件
      $getAll=isset($_GET['get_all'])?true:false;
      $getCheckout=$_GET['main_page']=='checkout'?true:false;

      if($getAll==false && $getCheckout==true || $_GET['main_page']=='checkout_process' || $_GET['main_page']=='checkout_success' || $_GET['min_page']=='address_book_edit'){
        //筛选勾选产品
        foreach($data as $k=>$v) {
          for($i=0;$i<$count;$i++){
            if($k==$sessionIds[$i]){
      ...
      while (list($products_id, ) = each($data)) {
        ...
        if ($product->RecordCount()<=0) {
          ...
          if (isset($this->contents[$products_id]['attributes'])) {
            ...
            while (list($option, $value) = each($this->contents[$products_id]['attributes'])) {
              ...
              if($option == 'length'){
                $sql =  "insert  into customers_basket_length  (customers_id,products_id,length_name,product_length_id) values ('".(int)$_SESSION['customer_id']."','" . zen_db_input($products_id) . "','$option','$value')";
                $db->query($sql);
              }else{
    ...
    while (!$products->EOF) {
      ...
      $length_list = $db->getAll("select * from customers_basket_length where products_id = '".$products->fields['products_id']."' limit 1");
      if($length_list){
  ...
  function reset($reset_database = false) {
    ...
    if(!empty($sessionIds)){
      ...
      if(!empty($_SESSION['shortCart'])){
        $this->contents = $_SESSION['shortCart'];
      }
      foreach ($this->contents as $k=>$v){
        ...
        if($a==$k){
          unset($this->contents[$a]);
        }
      }
      unset($_SESSION['shopping_pro_id']);
    }else{
      $this->contents = array();
    }
...
//biaoji
   ...
   */
  function add_cart($products_id, $qty = '1', $attributes = '', $notify = true,$id_column = array()) {
    global $db;
    ...
    if ($this->in_cart($products_id)) {
      ...
      $this->contents[$products_id]['fiber_count'] = set_fibers_count_attribute($products_id,$attributes);
...
//  }
  function remove_all($products_ids) {
    $this->notify('NOTIFIER_CART_REMOVE_ALL_START');
    //$this->reset();
  ...
  function calculate() {
    ...
    $count=count($sessionIds);
    if($getAll==false && $getCheckout==true || $_GET['main_page']=='checkout_process' || $_GET['main_page']=='checkout_success' || $_GET['min_page']=='address_book_edit'){
      if($count==0 && $_GET['main_page']!='checkout_success' ){
//        zen_redirect('http://test.whgxwl.com:8000/index.php?main_page=shopping_cart');
      }else{
        //筛选勾选产品
        foreach($data as $k=>$v) {
          for($i=0;$i<$count;$i++){
...
//      $product_discount_query="select products_id,discount_qty,discount_price
//                         from ". TABLE_PRODUCTS_DISCOUNT_QUANTITY ."
//                         where products_id = '" . (int)$products_id . "'";
//
//     $product_discount=$db->Execute($product_discount_query);
      //var_dump($product_discount->fields['products_id']);
...
//         if($product_discount->fields['products_id']){
//         $total = zen_add_tax($products_price, $products_tax) * $qty;
//
//          }else{
//         $total = $fiberstore_discount->fiberstore_products_discount($products_price,$qty);
//
//          }
//var_dump(get_products_have_discount_of_categories((int)$product->fields['products_id']));


//        $transceiver_id = get_all_categories_for_transceivers((int)$product->fields['products_id']);
//        if($transceiver_id || get_all_categories_for_fiber_cable((int)$product->fields['products_id']) ){
//        $products_price = zen_get_products_discount_price_qty($product->fields['products_id'], $qty);
//        $total = zen_add_tax($products_price, $products_tax) * $qty;
//        }
//        else
        $currencies_value = zen_get_currencies_value_of_code($_SESSION['currency']);
        if(get_products_have_discount_of_categories((int)$product->fields['products_id'])){

          if(isset($_SESSION['member_level'])){
            $level_products_price = get_customers_products_level_price(($products_price+$this->attributes_price($products_id)),$_SESSION['member_level']);
            if(!in_array($products_id,$wholesale_products)){
              $level_products_price = $currencies->new_value($level_products_price);
            }else{
              $level_products_price = $currencies->value($level_products_price);
            }
          }else{
            $level_products_price = $products_price+$this->attributes_price($products_id);
            if(!in_array($products_id,$wholesale_products)){
              $level_products_price = $currencies->new_value($level_products_price);
            }else{
              $level_products_price = $currencies->value($level_products_price);
          ...
          $total = zen_add_tax( $products_price, $products_tax) * $qty;
          $level_total = $level_products_price * $qty;
        }

        else{
          $total = $fiberstore_discount->fiberstore_products_discount($products_price,$product->fields['products_id'],$qty);
        }//var_dump($product->fields['products_id']);
        //var_dump($total);
        $this->total +=$total;
...
// appears to confuse products priced by attributes
                ...
                if ($attribute_price->fields['attributes_discounted'] == '1') {
                  // calculate proper discount for attributes
                  $new_attributes_price = zen_get_discount_calc($product->fields['products_id'], $attribute_price->fields['products_attributes_id'], $attribute_price->fields['options_values_price'], $qty);
                  if(get_outer_jacket_options_values_price($option,$new_attributes_price,$length_s)){
                    $new_attributes_price = get_outer_jacket_options_values_price($option,$new_attributes_price,$length_s);
                  }
                  if($re = fs_attribute_column_option_value_price($products_id,$this->columnID,$option,(int)$value)){
                    $new_attributes_price1 = $re[0];

                    if($re[1] == '+'){
                      $this->total += $qty * zen_add_tax( ($new_attributes_price1), $products_tax);
                    }else{
                      $this->total -= $qty * zen_add_tax( ($new_attributes_price1), $products_tax);
                    }
                  ...
                  if(get_outer_jacket_options_values_price($option,$attribute_price->fields['options_values_price'],$length_s)){
...
//  echo 'I SEE ' . $this->total . ' vs ' . $this->free_shipping_price . ' items: ' . $this->free_shipping_item. '<br>';
            ...
            if(pulling_eye_status($option)){
              $this->total += PULLINGEYENUM*(int)$this->contents[$products_id]['attributes_values'][$option];
            }
            if(isset($this->contents[$products_id]['fiber_count']['option_id'])){
              if($option == $this->contents[$products_id]['fiber_count']['option_id']){
                $this->total += $qty*($this->contents[$products_id]['fiber_count']['options_values_price']);
                $this->weight += $qty*($this->contents[$products_id]['fiber_count']['products_attributes_weight']);
              }
            }
          }else{
            //length
            $list = $db->getAll("select * from products_length where id = '$value' and product_id = '". (int)$products_id ."'");
            if($list){
              if($list[0]['price_prefix'] == '+'){
                $this->total += $qty*get_discount_price($list[0]['length_price'],$qty,(int)$products_id);
                $this->weight += $qty*$list[0]['weight'];
              }elseif($list[0]['price_prefix'] == '-'){
                $this->total -= $qty*$list[0]['length_price'];
                $this->weight -= $qty *$list[0]['weight'];
              }
            }
          }
      ...
      while (list($option, $value) = each($this->contents[$products_id]['attributes'])) {
        ...
        if($option != 'length'){
          ...
          if ($attribute_price->fields['product_attribute_is_free'] == '1' and zen_get_products_price_is_free((int)$products_id)) {
            ...
            if(pulling_eye_status($option)){
              $attributes_price += PULLINGEYENUM*(int)$this->contents[$products_id]['attributes_values'][$option];
            }
            //////////////////////////////////////////////////
          }
  ...
  function get_products($check_for_valid_cart = false) {
    ...
    while (list($products_id, ) = each($data)) {
      ...
      if ($products = $db->Execute($products_query)) {
        ...
        $wholesale_price = fs_get_product_wholesale_price_of_qty((int)$products->fields['products_id'],$new_qty);
...
// validate cart contents for checkout
        ...
        if(!in_array($products_id,$wholesale_products)){
          ...
          $products_price_start = $products_price;
          $products_price = $products_price*$currencies_value;
        }

        if(isset($_SESSION['member_level'])){
          $products_prices = get_customers_products_level_price($products_price+$this->attributes_price($products_id)*$currencies_value,$_SESSION['member_level']);
        ...
        $products_price_original = $products_price/$currencies_value+$this->attributes_price($products_id);
        ...
        $products_array[] = array('id' => $products_id,
          ...
          // fix here
          //'products_price' => $products->fields['products_price'],
            'products_price' => $products_price_original,
...
//          $messageStack->add('header', 'REMOVE ME IN SHOPPING CART CLASS BEFORE RELEASE<br/><BR />' . ERROR_CORRECTIONS_HEADING . $the_list, 'error');
          ...
          if(isset($_POST['custom_length']) && $_POST['custom_length']){
            if(isset($_POST['custom_length']) && is_numeric($_POST['custom_length']) && ($_POST['custom_length'])>0){
              ...
              $result = $db->getAll("select id from products_length where product_id = '".$_POST['products_id']."' and (length = '".trim($_POST['custom_length'])."m' OR length = '".trim($_POST['custom_length'])."km') limit 1");
              if($result){
                $length = $result[0]['id'];
              }else{
                  ...
                  if($list){
                    if($_POST['custom_length']>1){
                      ...
                      if($key == 0){
                        ...
                        $weight = ($_POST['custom_length']-1)*$list[0]['unit_weight'];
                        $length_price = round($length_price,2);
                        $weight = round($weight,2);
                        $_POST['custom_length'] = round($_POST['custom_length'],2);
                        $db->query("insert into products_length (length_price,price_prefix,weight,length,product_id,add_time,sign,custom) values ('".$length_price."','+','".$weight."','".$_POST['custom_length']."m','".$_POST['products_id']."','".date('Y-m-d H:i:s')."','0','1')");
                      }elseif($key == 1){
                        if($retail == 1){
                          $length_price = ($_POST['custom_length']-1)*$list[0]['unit_price'];
                          $weight = ($_POST['custom_length']-1)*$list[0]['unit_weight'];
                          ...
                          $weight = ($_POST['custom_length']*1000-1)*$list[0]['unit_weight'];
                          $length_price = round($length_price,2);
                          $weight = round($weight,2);
                          $_POST['custom_length'] = round($_POST['custom_length'],2);
                          $db->query("insert into products_length (length_price,price_prefix,weight,length,product_id,add_time,sign,custom) values ('".$length_price."','+','".$weight."','".$_POST['custom_length']."km','".$_POST['products_id']."','".date('Y-m-d H:i:s')."','0','1')");
                        }
                      }elseif($key == 2){
                        $length_price = ($_POST['custom_length']*1000-1)*$list[0]['unit_price'];
                        $weight = ($_POST['custom_length']*1000-1)*$list[0]['unit_weight'];
                        ...
                        $custom_length =  $_POST['custom_length']-10>0 ? $_POST['custom_length']-10 :0;
                        ...
                        $custom_length =  $_POST['custom_length']-100>0 ? $_POST['custom_length']-100 :0;
                        $length_price = ($custom_length)*$list[0]['unit_price'];
                        $weight = ($custom_length)*$list[0]['unit_weight'];
                        $length_price = round($length_price,2);
                        $weight = round($weight,2);
                        ...
                        $custom_length =  $_POST['custom_length']-3>0 ? $_POST['custom_length']-3 :0;
                        $length_price = ($custom_length)*$list[0]['unit_price'];
                        $weight = ($custom_length)*$list[0]['unit_weight'];
                        $length_price = round($length_price,2);
                        $weight = round($weight,2);
                      ...
                      if($key==2 || ($key==1 && $retail==0)){
                        ...
                        $weight = (1000-1)*$list[0]['unit_weight'];
                      }else{
                        $length_price = 0;
                        $weight = 0;
                      }
                      ...
                      $weight = round($weight,2);
                      $_POST['custom_length'] = round($_POST['custom_length'],2);
                      $db->query("insert into products_length (length_price,price_prefix,weight,length,product_id,add_time,sign,custom) values ('".$length_price."','+','".$weight."','".trim($_POST['custom_length'])."m','".$_POST['products_id']."','".date('Y-m-d H:i:s')."','0','1')");
                    }
                  }
                }
                $length = mysql_insert_id();
              }
            }
          ...
          if (isset($_GET['number_of_uploads']) && $_GET['number_of_uploads'] > 0) {
            ...
            for ($i = 1, $n = $_GET['number_of_uploads']; $i <= $n; $i++) {
              if (zen_not_null($_FILES['id']['tmp_name'][TEXT_PREFIX . $_POST[UPLOAD_PREFIX . $i]]) and ($_FILES['id']['tmp_name'][TEXT_PREFIX . $_POST[UPLOAD_PREFIX . $i]] != 'none')) {
                ...
                $products_options_file->set_destination(DIR_FS_PIC_UPLOADS);
